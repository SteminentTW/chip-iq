...
| 籌碼分析 | 券商分點進出排名（區間可選 當日／5／10／20／60／120 日／全部），每家附「區間前部位／成本／動作」；扣除造市商後的真實換手拆解；近 20 日持續累積／出貨；持股成本分層（潛在賣壓） | 實際資料 |
...
期間有兩組代號整批換掉，兩件都有官方來源，站上自 v0.6 起**依對照表併為同一分點**，不再只是警告：

| 舊代號 | 新代號 | 生效 | 規則 | 來源 |
|---|---|---|---|---|
| `592x`（元富） | `9B2x` | 2026-02-02 | 後一碼不變 | [元富證券公告 115-01-05](https://www.masterlink.com.tw/936/) |
| `815x`（台新） | `9B1x` | 2026-04-07（合併基準日 4/6 的次一交易日） | **後一碼有變**，依分公司名稱對應（8150→9B17 營業部、8151→9B18 建北…） | [台新證券合併專區・服務據點異動](https://www.tssco.com.tw/TSHOLDINGSMERGE/locations/index.html) |
...
全部由前端從每日明細即時算，沒有新資料源：

- **區間前部位／成本／動作**：每家分點在區間起始日之前、自 2025-07-28 累計的淨額與成交量加權成本；本區間動作分 加碼／回補／新進（買方）、了結／認賠／續出／新賣（賣方），了結與認賠以區間賣出均價對其區間前成本判定。
- **真實換手**：造市商經手固定約佔單邊量一半（全期中位數，興櫃議價結構），先扣掉，再把非造市商淨買超與淨賣超各拆來源。
- **持續累積／持續出貨**：固定近 20 個交易日，同方向淨額天數 ≥ 5、反向量 ≤ 同向量 25%、淨額 ≥ 股本 0.05%；最長連買以交易日計、停手即中斷。
- **持股成本分層**：累計仍為淨買超的分點按成本每 10 元一層，對區間最後一日均價分獲利／套牢，另給獲利籌碼比率與本區間已了結量。

限制寫在畫面上：更早於 2025-07-28 的持股不在內；分點是券商據點不是單一投資人；成本是據點層級的近似值。
